...
class ReservedSessionsBody extends StatelessWidget {
  const ReservedSessionsBody({super.key});
        ...
        BlocBuilder<HomeCubit, HomeState>(
          builder: (context, state) {
            if (state is HomeLoaded) {
              return Column(
                children: [
                  if (state.reservedSessions.isEmpty)
                    Padding(
                      padding: EdgeInsets.all(32.w),
                      child: Text(
                        'No reserved sessions',
                    ...
                    ...state.reservedSessions.map((sessionDetail) {
